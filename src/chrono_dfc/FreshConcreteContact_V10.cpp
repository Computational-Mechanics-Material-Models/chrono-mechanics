...
class ChMaterialFCM {
   ...
   public:
	...
	float beta=0.5f;
	float np=1.0f;
	float sgmTmax=9.0e-3f;
	float sgmTau0=5.0e-4f;
	float kappa0=100.0f;
	float eta_inf=10.0e-6f;
	float flocbeta=1.0e-2f;	
	float flocm=1.0f;
	float flocTcr=1000.0f;
	float lambda_init=1.0e-8f;
...
float ChGetShapeDimension(const ChCollisionShapeInstance& shape_instance){
...
class FCContactForce : public ChSystemSMC::ChContactForceTorqueSMC {
  public:
   ...
   virtual ChWrenchd CalculateForceTorque(
		...
		//
		//---------------------------
		// Get local frame (n,m,l)
		//---------------------------	

		ChMatrix33<> contact_plane;
		contact_plane.SetFromAxisX(normal_dir, VECT_Y);
		ChVector3d m = contact_plane.GetAxisY();
		ChVector3d l = contact_plane.GetAxisZ();
		
		//
		// Get the dimension of the object
		//	
		auto shapeA=bodyA->GetCollisionModel()->GetShapeInstance(0).shape->GetType();
		auto shapeB=bodyB->GetCollisionModel()->GetShapeInstance(0).shape->GetType();
		double viscous_shear_multiplier =
			((shapeA==ChCollisionShape::Type::SPHERE && shapeB==ChCollisionShape::Type::TRIANGLEMESH) ||
			 (shapeB==ChCollisionShape::Type::SPHERE && shapeA==ChCollisionShape::Type::TRIANGLEMESH)) ? 0.0 : 1.0;
		...
		/*if (bodyA->GetCollisionModel()->GetShapeInstance(0).shape->GetType()==0){        
			//R1=bodyA->GetCollisionModel()->GetShapeDimensions(0)[0];
			//R1=5;//bodyA->GetCollisionModel()->GetRadius();
			R1 = ChGetShapeDimension(bodyA->GetCollisionModel()->GetShapeInstance(0));	
			//auto shape=bodyA->GetCollisionModel()->GetShapeInstance(0).shape;
			//auto shape_sphere = std::static_pointer_cast<ChCollisionShapeSphere>(shape);
            //R1 = shape_sphere->GetRadius();				
		}
		
		if (bodyB->GetCollisionModel()->GetShapeInstance(0).shape->GetType()==0){        
			//R2=bodyB->GetCollisionModel()->GetShapeDimensions(0)[0];
			//R2=5;//bodyA->GetCollisionModel()->GetRadius();
			//auto shape=bodyB->GetCollisionModel()->GetShapeInstance(0).shape;
			//auto shape_sphere = std::static_pointer_cast<ChCollisionShapeSphere>(shape);
		...
		/*if (bodyA->GetCollisionModel()->GetShapeInstance(0).shape->GetType()!=0 || 
						bodyB->GetCollisionModel()->GetShapeInstance(0).shape->GetType()!=0){ 
			// contact of DFC sphere with outer wall surface
		...
		*/
        ...
        double relvel_n_mag = relvel.Dot(normal_dir);
        ChVector3d relvel_n = relvel_n_mag * normal_dir;
		double relvel_m = relvel.Dot(m);
		double relvel_l = relvel.Dot(l);
        //ChVector3d relvel_t = relvel - relvel_n;
        //double relvel_t_mag = relvel_t.Length();		
		//
		...
		double delta_m = relvel_m * dT; //Vy.Dot(relvel_t) * dT;
        double delta_l = relvel_l * dT; //Vz.Dot(relvel_t) * dT;ChVector3d v_delta_t = relvel_t * dT;
		//
		...
		[[maybe_unused]] double epsQ=pow(epsN*epsN+alpha*epsT*epsT,0.5);
		//
		//
		//
		...
		if (epsN<0){
			...
			if (epsN>=epsA){				
				...
				// Deep compression: aggregate stiffness contribution disabled.
			...
			double sgmM_vis=viscous_shear_multiplier*eta*vdepsM;
			double sgmL_vis=viscous_shear_multiplier*eta*vdepsL;
			...
			force=forceN*normal_dir+forceM*m+forceL*l;
...
class MyContactContainer : public ChContactContainerSMC {
  public:
    MyContactContainer() {}
    ~MyContactContainer() {}
    // Traverse contacts through the Chrono 10 public report callback API
	...
	struct LambdaReporter : public ChContactContainer::ReportContactCallback {
		LambdaReporter(int& num_contact,
			...
			: num_contact(num_contact),
			  ...
			  ie(ie),
			  updated_map_contact_info(updated_map_contact_info) {}
		...
		virtual bool OnReportContact(const ChVector3d& pA,
			...
			ChVector3d force = plane_coord * react_forces;
			ChVector3d dp = pA - pB;
			...
			double lambda = map_contact_info[mykey].lambda;
			if (lambda > 0)
	...
	ReportAllContacts(chrono_types::make_shared<LambdaReporter>(num_contact, AverageLambda, IE, updated_map_contact_info));
	...
	std::unordered_map<std::string, MyStateVar> updated_map_contact_info;
	struct ParticleReporter : public ChContactContainer::ReportContactCallback {
		ParticleReporter(int& num_contact,
						 double& ie,
						 std::unordered_map<std::string, MyStateVar>& updated_map_contact_info)
			: num_contact(num_contact), ie(ie), updated_map_contact_info(updated_map_contact_info) {}
	...
	ReportAllContacts(chrono_types::make_shared<ParticleReporter>(num_contact, IE, updated_map_contact_info));
	...
	int TrackHardContact(double& h_layer) {
		...
		double max_CD=0;
		struct HardContactReporter : public ChContactContainer::ReportContactCallback {
			HardContactReporter(double& h_layer, int& num_hard_contact, double& max_CD)
				: h_layer(h_layer), num_hard_contact(num_hard_contact), max_CD(max_CD) {}
			...
			virtual bool OnReportContact(const ChVector3d& pA,
				...
				if (distance < max_CD)
					max_CD = distance;
				if (distance <= -2 * h_layer)
					num_hard_contact++;
				return true;
			}

			double& h_layer;
			int& num_hard_contact;
			double& max_CD;
		};

		ReportAllContacts(chrono_types::make_shared<HardContactReporter>(h_layer, num_hard_contact, max_CD));
...
void WriteParticlesVTK(ChSystem& sys, const std::string& filename, double h_layer, bool isAgregateOut) {
	 ...
	 for (auto body:body_list){
	         if (body->GetCollisionModel() && !body->IsFixed() && body->GetCollisionModel()->GetShapeInstance(0).shape->GetType()==0) {
	...
	int num_particles = body_list_new.size();
     ...
     for (int i = 0; i < num_particles; i++) {
		auto shape=body_list_new[i]->GetCollisionModel()->GetShapeInstance(0).shape;
		auto shape_sphere = std::static_pointer_cast<ChCollisionShapeSphere>(shape);
        double Radius = shape_sphere->GetRadius();	
		if (isAgregateOut)

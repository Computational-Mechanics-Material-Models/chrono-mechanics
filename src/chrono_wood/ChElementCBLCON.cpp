...
void ChElementCBLCON::ElementUpdateEndStep(double time) {
    // Commit the converged trial state only after Chrono accepts the time step.
    // Do NOT do this in ComputeInternalForces(), because that function can be called
    // multiple times during nonlinear iterations within the same time step.
    if (statevar.size() != GetNumStateVar()) {
        statevar.setZero(GetNumStateVar());
        GetStateBlock(statevar);
    } else {
        // Keep the committed DOF part synchronized with the accepted nodal state.
        GetStateBlock(statevar);
    }
...
void ChElementCBLCON::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    // IMPORTANT: start every residual/tangent evaluation from the committed state.
    // ComputeStress() may modify statevar as a trial state during Newton iterations;
    // resetting here prevents trial internal variables from being accumulated every iteration.
    if (statevar_old.size() != GetNumStateVar()) {
        statevar_old.setZero(GetNumStateVar());
        GetStateBlock(statevar_old);
    }
    statevar = statevar_old;
    this->GetStateBlock(statevar); // Store current DOFs at the end of the trial state vector.

...
ChColor ChVisualShapeWOOD::ComputeFalseColor(double mv) {
    if (fem_data_type == DataType::SURFACE)
        return meshcolor;
    return ComputeFalseColor_v10(mv, colorscale_min, colorscale_max, true);
    //return ChColor::ComputeFalseColor_v10(mv, colorscale_min, colorscale_max, true);
}

ChColor ChVisualShapeWOOD::ComputeFalseColor_v10(double v,
                                          double vmin,
                                          double vmax,
                                          bool out_of_range_as_bw) {
    if (out_of_range_as_bw) {
        if (v < vmin)
            return ChColor(0.0f, 0.0f, 0.0f);  // black
        if (v > vmax)
            return ChColor(1.0f, 1.0f, 1.0f);  // white
    }
    ...
    if (dv <= 0.0)
        return ChColor(1.0f, 1.0f, 1.0f);

    if (v < vmin + 0.25 * dv) {
        double f = (v - vmin) / (0.25 * dv);

        return ChColor::Interp(
            ChColor(0.0f, 0.0f, 1.0f),  // blue
            ChColor(0.0f, 1.0f, 1.0f),  // cyan
            f
        );
    } else if (v < vmin + 0.50 * dv) {
        double f = (v - (vmin + 0.25 * dv)) / (0.25 * dv);

        return ChColor::Interp(
            ChColor(0.0f, 1.0f, 1.0f),  // cyan
            ChColor(0.0f, 1.0f, 0.0f),  // green
            f
        );
    } else if (v < vmin + 0.75 * dv) {
        double f = (v - (vmin + 0.50 * dv)) / (0.25 * dv);

        return ChColor::Interp(
            ChColor(0.0f, 1.0f, 0.0f),  // green
            ChColor(1.0f, 1.0f, 0.0f),  // yellow
            f
        );
    } else {
        double f = (v - (vmin + 0.75 * dv)) / (0.25 * dv);

        return ChColor::Interp(
            ChColor(1.0f, 1.0f, 0.0f),  // yellow
            ChColor(1.0f, 0.0f, 0.0f),  // red
            f
        );
    }
}

...
class ChWoodApi ChElementCBLCON : public ChElementBeam,				
  ...
  public:
    ...
    //virtual unsigned int GetNumStateVar() override { return section->Get_material()->GetNumberOfStateVariables() + 2*7; }
    unsigned int GetNumStateVar() { return section->Get_material()->GetNumberOfStateVariables() + 2*7; } // TODO: the + 2*7 is designed to store the old DOFs (position (3) + quaternion (4)) in order to compute strain increment.
    ...
    /// Update element internal variables at the end of an accepted time step.
  ...
  public: // TODO JBC: WHY IS ALL OF THAT PUBLIC ?!?! I Think some of it must be private, let's discuss!
    ...
    // Trial and committed internal state vectors.
    // statevar_old is the committed state from the last accepted time step.
    // statevar is rebuilt from statevar_old in ComputeInternalForces() and used as the trial state.

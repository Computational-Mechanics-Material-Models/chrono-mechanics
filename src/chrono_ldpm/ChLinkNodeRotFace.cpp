...
int  ChLinkNodeRotFace::Initialize(std::shared_ptr<ChNodeFEAxyzrot> anodeA,
    ...
    */
    ...
    constraint1.SetTuples(
        new ChConstraintTuple_1vars<6>(&m_node->Variables()),
        new ChConstraintTuple_3vars<6, 6, 6>(
            m_triangle.GetVariables1(),
            m_triangle.GetVariables2(),
            m_triangle.GetVariables3()
        )
    );

    constraint2.SetTuples(
        new ChConstraintTuple_1vars<6>(&m_node->Variables()),
        new ChConstraintTuple_3vars<6, 6, 6>(
            m_triangle.GetVariables1(),
            m_triangle.GetVariables2(),
    ...
    this->d = utils::PointTrianglePlaneDistance(m_node->GetPos(), m_triangle.node1->pos, m_triangle.node2->pos,  m_triangle.node3->pos, s2, s3, is_into, p_projected);
...
void  ChLinkNodeRotFace::Update(double mytime, UpdateFlags update_flags) {
    // Inherit time changes of parent class
    ChPhysicsItem::Update(mytime, update_flags);
...
void  ChLinkNodeRotFace::LoadConstraintJacobians() {
    ...
    */
    ...
    auto c1a = static_cast<ChConstraintTuple_1vars<6>*>(constraint1.TupleA());
    auto c2a = static_cast<ChConstraintTuple_1vars<6>*>(constraint2.TupleA());
    auto c3a = static_cast<ChConstraintTuple_1vars<6>*>(constraint3.TupleA());

    auto c1b = static_cast<ChConstraintTuple_3vars<6, 6, 6>*>(constraint1.TupleB());
    auto c2b = static_cast<ChConstraintTuple_3vars<6, 6, 6>*>(constraint2.TupleB());
    auto c3b = static_cast<ChConstraintTuple_3vars<6, 6, 6>*>(constraint3.TupleB());

    c1a->Cq1().segment(0, 3) = Jxa.row(0);
    c2a->Cq1().segment(0, 3) = Jxa.row(1);
    c3a->Cq1().segment(0, 3) = Jxa.row(2);

    c1b->Cq1().segment(0, 3) = Jxb1.row(0);
    c2b->Cq1().segment(0, 3) = Jxb1.row(1);
    c3b->Cq1().segment(0, 3) = Jxb1.row(2);

    c1b->Cq2().segment(0, 3) = Jxb2.row(0);
    c2b->Cq2().segment(0, 3) = Jxb2.row(1);
    c3b->Cq2().segment(0, 3) = Jxb2.row(2);

    c1b->Cq3().segment(0, 3) = Jxb3.row(0);
    c2b->Cq3().segment(0, 3) = Jxb3.row(1);
    c3b->Cq3().segment(0, 3) = Jxb3.row(2);
...
int  ChLinkNodeRotFaceRot::Initialize(std::shared_ptr<ChNodeFEAxyzrot> anodeA,
    ...
    */
    ...
    bool is_into;
    ChVector3d p_projected;
    this->d =utils::PointTrianglePlaneDistance(m_node->GetPos(), m_triangle.node1->GetPos(), m_triangle.node2->GetPos(), m_triangle.node3->GetPos(), s2, s3, is_into, p_projected);
...
void  ChLinkNodeRotFaceRot::Update(double mytime, UpdateFlags update_flags) {    
    // Inherit time changes of parent class
    ChPhysicsItem::Update(mytime, update_flags);
...
void  ChLinkNodeRotFaceRot::LoadConstraintJacobians() {
    ...
    */
    ...
    auto c4a = static_cast<ChConstraintTuple_1vars<6>*>(constraint4.TupleA());
    auto c5a = static_cast<ChConstraintTuple_1vars<6>*>(constraint5.TupleA());
    auto c6a = static_cast<ChConstraintTuple_1vars<6>*>(constraint6.TupleA());

    auto c4b = static_cast<ChConstraintTuple_3vars<6, 6, 6>*>(constraint4.TupleB());
    auto c5b = static_cast<ChConstraintTuple_3vars<6, 6, 6>*>(constraint5.TupleB());
    auto c6b = static_cast<ChConstraintTuple_3vars<6, 6, 6>*>(constraint6.TupleB());

    c4a->Cq1().segment(3, 3) = Jxa.row(0);
    c5a->Cq1().segment(3, 3) = Jxa.row(1);
    c6a->Cq1().segment(3, 3) = Jxa.row(2);

    c4b->Cq1().segment(3, 3) = Jw1.row(0);
    c5b->Cq1().segment(3, 3) = Jw1.row(1);
    c6b->Cq1().segment(3, 3) = Jw1.row(2);

    c4b->Cq2().segment(3, 3) = Jw2.row(0);
    c5b->Cq2().segment(3, 3) = Jw2.row(1);
    c6b->Cq2().segment(3, 3) = Jw2.row(2);

    c4b->Cq3().segment(3, 3) = Jw3.row(0);
    c5b->Cq3().segment(3, 3) = Jw3.row(1);
    c6b->Cq3().segment(3, 3) = Jw3.row(2);
}

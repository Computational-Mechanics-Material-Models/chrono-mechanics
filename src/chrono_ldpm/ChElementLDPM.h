...
class ChLdpmApi ChElementLDPM : public ChElementTetrahedron_6DOFs,
  ...
  private:    
    ...
    std::vector<ChVectorDynamic<>> statev_old; // for storing the state variables at the end of the previous time step, to be used in the updated Lagrangian formulation until these are stored at the nodes in Tasora's new design
    std::vector<ChVectorDynamic<>> statev_new; // for storing the state variables at the end of the current time step, to be used in the updated Lagrangian formulation until these are stored at the nodes in Tasora's new design

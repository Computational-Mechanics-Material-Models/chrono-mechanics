...
void ChElementLDPM::SetupInitial(ChSystem* system) {
	...
	// Prepare old committed state variable storage.
    statev_old.clear();
    statev_old.resize(this->GetSection().size());
	statev_new.clear();
    statev_new.resize(this->GetSection().size());
...
void ChElementLDPM::ComputeInternalForces(ChVectorDynamic<>& Fi) {
    ...
    //
    //
	//std::cout<<"compute stress on facets: \n";
    unsigned int iface=0;
    for (auto facet:this->GetSection()){    
...
void ChElementLDPM::ElementUpdateEndStep(double time) {
	//std::cout << "ElementUpdateEndStep called at time = " << time << std::endl;
    for (unsigned int iface = 0; iface < this->GetSection().size(); ++iface) {
        auto facet = this->GetSection()[iface];

        facet->Set_StateVar(statev_new[iface]);
        statev_old[iface] = statev_new[iface];
    }
...
void ChElementLDPM::ElementUpdateEndStep(double time) {
	std::cout << "ElementUpdateEndStep called at time = " << time << std::endl;
    // Recompute final accepted displacement increment.
    ChVectorDynamic<> displ(24);
    this->GetStateBlock(displ);
    DUn_1 = displ - Un_1;
    ...
    for (auto facet : this->GetSection()) {
        ...
        // Start from old committed state.
        statev = statev_old[iface];
        ...
        // Commit only once at end of accepted step.
        facet->Set_StateVar(statev);
        statev_old[iface] = statev;
    ...
    // Now commit the accepted displacement.
    Un_1 = displ;
    DUn_1.setZero(24);
}
*/

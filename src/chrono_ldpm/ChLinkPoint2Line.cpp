...
int ChLinkPoint2Line::Initialize(std::shared_ptr<ChNodeFEAxyz> anodeA,
    ...
    */
    ...
    constraint1.SetTuples(
        new ChConstraintTuple_1vars<6>(&mnodeA->Variables()),
        new ChConstraintTuple_2vars<6, 6>(
            mLine.GetVariables1(),
            mLine.GetVariables2()
        )
    );
...
void ChLinkPoint2Line::Update(double mytime, UpdateFlags update_flags) {
...
void ChLinkPoint2Line::LoadConstraintJacobians() {
    ...
    */
    ...
    auto c1b = static_cast<ChConstraintTuple_2vars<6, 6>*>(constraint1.TupleB());
    auto c2b = static_cast<ChConstraintTuple_2vars<6, 6>*>(constraint2.TupleB());
    auto c3b = static_cast<ChConstraintTuple_2vars<6, 6>*>(constraint3.TupleB());
...
void ChLinkPoint2LineRot::Update(double mytime, UpdateFlags update_flags) {
    // Inherit time changes of parent class
    ChPhysicsItem::Update(mytime, update_flags);
...
void ChLinkPoint2LineRot::LoadConstraintJacobians() {
    ...
    */
    ...
    auto c4b = static_cast<ChConstraintTuple_2vars<6, 6>*>(constraint4.TupleB());
    auto c5b = static_cast<ChConstraintTuple_2vars<6, 6>*>(constraint5.TupleB());
    auto c6b = static_cast<ChConstraintTuple_2vars<6, 6>*>(constraint6.TupleB());

    c4a->Cq1().segment(0, 3) = Jxa.row(0);
    c5a->Cq1().segment(0, 3) = Jxa.row(1);
    c6a->Cq1().segment(0, 3) = Jxa.row(2);

    c4b->Cq1().segment(0, 3) = Jxb1.row(0);
    c5b->Cq1().segment(0, 3) = Jxb1.row(1);
    c6b->Cq1().segment(0, 3) = Jxb1.row(2);

    c4b->Cq2().segment(0, 3) = Jxb2.row(0);
    c5b->Cq2().segment(0, 3) = Jxb2.row(1);
    c6b->Cq2().segment(0, 3) = Jxb2.row(2);
}

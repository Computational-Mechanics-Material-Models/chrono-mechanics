...
int main(int argc, char** argv) {
    ...
    //std::string size_folder = "/size" + std::to_string(static_cast<int>(height));
    ...
    ///
    ///
    auto bottom_plate = chrono_types::make_shared<ChBodyEasyBox>(100, 100, 30, 7.8E-9, true, true, mysurfmaterial);
    bottom_plate->SetPos(ChVector3d(25, 25, insert-15));
    ...
    constr_bot_plate->Initialize(bottom_plate, mtruss, bottom_plate->GetFrameCOMToAbs());
    sys.Add(constr_bot_plate);

    auto top_plate = chrono_types::make_shared<ChBodyEasyBox>(100, 100, 30, 7.8E-9, true, true, mysurfmaterial);
    top_plate->SetPos(ChVector3d(25, 25, height + 15 - insert));
    top_plate->GetVisualShape(0)->SetTexture("/chrono-concrete/data/textures/blue.png");
    sys.Add(top_plate);
    */
    

    //auto constr_top_plate = chrono_types::make_shared<ChLinkMateGeneric>(true, true, true, true, true, true);
    ...
    my_mat->Set_sigmat(4.5);
    my_mat->Set_sigmas(1.8);
    my_mat->Set_nt(0.6);
    my_mat->Set_lt(1000);
    my_mat->Set_Ed(50000);
    my_mat->Set_sigmac0(100);
    my_mat->Set_Hc0(25000);
    my_mat->Set_Hc1(5000);
    my_mat->Set_beta(0);
    my_mat->Set_kc0(5);
    my_mat->Set_kc1(1);
    ...
    my_mat->Set_mu0(0.95);
    my_mat->Set_muinf(0);
    my_mat->Set_sigmaN0(50);
	
	my_mat->Set_ksp(6.2);
    my_mat->Set_ksn(0.5);
    my_mat->Set_Ef(200000*0.7);
    my_mat->Set_t0(5.1);
    my_mat->Set_Gd(0.0);
    my_mat->Set_beta_fiber(-0.05);
    my_mat->Set_sigmauf(1000);
    my_mat->Set_krup(0);
    ...
    builder.read_LDPM_info(my_mesh, my_mat, LDPM_data_path, LDPM_GeoName, true);
    ...
    //std::shared_ptr<ChNodeFEAxyzrot> top_node1;
    std::shared_ptr<ChNodeFEAxyzrot> top_node2;
    //std::shared_ptr<ChNodeFEAxyzrot> top_node3;
    std::shared_ptr<ChNodeFEAxyzrot> top_node4;
    //std::shared_ptr<ChNodeFEAxyzrot> bot_node1;
    std::shared_ptr<ChNodeFEAxyzrot> bot_node2;
    //std::shared_ptr<ChNodeFEAxyzrot> bot_node3;
    std::shared_ptr<ChNodeFEAxyzrot> bot_node4;
    ...
    //double min_dist11 = 100;
    double min_dist22 = 15;
    //double min_dist33 = 100;
    double min_dist44 = 15;

    double min_dist_top = 30;
    double min_dist_bot = 30;
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        ...
        if (abs(p.z()-height-thick*2) < 0.001) {
            double dist = (p.x() - 60) * (p.x() - 60) + (p.y() - 10) * (p.y() - 10);
            if (dist < min_dist_top) {
                RP1 = node;
                min_dist_top = dist;
                std::cout << "find new RP1" << std::endl;
            }
        }
        ...
        if (abs(p.z()) < 0.001) {
            ...
            if (dist < min_dist_bot) {
                RP2 = node;
                min_dist_bot = dist;
                std::cout << "find new RP2" << std::endl;
            }
        }

        /*
        if (abs(p.z()) < 0.001 && abs(p.x() - 50) < 2 && abs(p.y() - 25) < 2) {
            RP2 = node;
        }

        if (abs(p.z() - 300) < 0.001 && abs(p.x() - 50) < 2 && abs(p.y() - 25) < 2) {
            RP1 = node;
        }
        */
        ...
        /*
        if (abs(p.y()) < 0.001) {
            double dist1 = (height + thick - p.z())* (height + thick - p.z()) + (p.x() - 50) * (p.x() - 50);
            if (dist1 < min_dist1) {
                top_node1 = node;
                min_dist1 = dist1;
                std::cout << "find new top1" << std::endl;
            }
        }
        */
        ...
        if (abs(p.x()-110) < 1) {
            double dist2 = (height + thick - p.z()) * (height + thick - p.z()) + (p.y() - 10) * (p.y() - 10);
            if (dist2 < min_dist2) {
                top_node2 = node;
                min_dist2 = dist2;
                std::cout << "find new top2" << std::endl;
            }
        }

        /*
        if (abs(p.y()-50) < 0.001) {
            double dist3 = (300 - thick - p.z()) * (300 - thick - p.z()) + (p.x() - 50) * (p.x() - 50);
            if (dist3 < min_dist3) {
                top_node3 = node;
                min_dist3 = dist3;
                std::cout << "find new top3" << std::endl;
            }
        }
        */
        ...
        if (abs(p.x() - 10) < 1) {
            double dist4 = (height + thick - p.z()) * (height + thick - p.z()) + (p.y() - 10)* (p.y() - 10);
            if (dist4 < min_dist4) {
                top_node4 = node;
                min_dist4 = dist4;
                std::cout << "find new top4" << std::endl;
            }
        }
        ...
        /*
        if (abs(p.y()) < 0.001) {
            double dist1 = (p.z() - thick) * (p.z() - thick) + (p.x() - 50) * (p.x() - 50);
            if (dist1 < min_dist11) {
                bot_node1 = node;
                min_dist11 = dist1;
                std::cout << "find new bot1" << std::endl;
            }
        }
        */
        ...
        if (abs(p.x() - 110) < 1) {
            double dist2 = (p.z() - thick) * (p.z() - thick) + (p.y() - 10) * (p.y() - 10);
            if (dist2 < min_dist22) {
                bot_node2 = node;
                min_dist22 = dist2;
                std::cout << "find new bot2" << std::endl;
            }
        }

        /*
        if (abs(p.y() - 50) < 0.001) {
            double dist3 = (p.z() - thick) * (p.z() - thick) + (p.x() - 50) * (p.x() - 50);
            if (dist3 < min_dist33) {
                bot_node3 = node;
                min_dist33 = dist3;
                std::cout << "find new bot3" << std::endl;
            }
        }
        */
        ...
        if (abs(p.x() - 10) < 1) {
            double dist4 = (p.z() - thick) * (p.z() - thick) + (p.y() - 10) * (p.y() - 10);
            if (dist4 < min_dist44) {
                bot_node4 = node;
                min_dist44 = dist4;
                std::cout << "find new bot4" << std::endl;
            }
        }
    ...
    std::cout << "finish find nodes" << std::endl;
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        ...
        auto p=node->GetPos();
        double tol = 0.001;
        if (abs(p.z() - height-thick*2) < tol && p != RP1->GetPos()) {
            top_nodes.push_back(node);
			continue;
        } 
		if ((p.z() - height-thick*1.4) > 0 && p != RP1->GetPos()){
			if (abs(p.x()) < tol || abs(p.x()-120) < tol || abs(p.y()) < tol || abs(p.y()-20) < tol){
				top_nodes.push_back(node);
			}
		}
    }

    std::cout << "finish find top nodes" << std::endl;
    //
    ///////////////////////////////////////////////////////////////////////////////////////////////////////
    ///
    ...
    for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
        ...
        auto p=node->GetPos();
        double tol = 0.001;
        if (abs(p.z()) < tol && p != RP2->GetPos()) {
            bottom_nodes.push_back(node);
			continue;
        }
		if ((p.z() -thick*0.6) < 0 && p != RP1->GetPos()){
			if (abs(p.x()) < tol || abs(p.x()-120) < tol || abs(p.y()) < tol || abs(p.y()-20) < tol){
				bottom_nodes.push_back(node);
			}
		}
    ...
    std::cout << "finish find bot nodes" << std::endl;



    std::cout << "RP1 " << RP1->GetPos() << std::endl;
    std::cout << "RP2 " << RP2->GetPos() << std::endl;
    std::cout << "TOP nodes number: " << top_nodes.size() << std::endl;
    std::cout << "BOT nodes number: " << bottom_nodes.size() << std::endl;
    std::cout << " top_node" << top_node2->GetPos() << "  " << top_node4->GetPos() << std::endl;
    std::cout << " bot_node" << bot_node2->GetPos() << "  " << bot_node4->GetPos() << std::endl;
	...
	*/  
    ...
    ///
        ...
        mystepper->SetAbsTolerances(1, 1);
    ...
    auto my_motion_function2 = chrono_types::make_shared<ChFunctionRamp>(0, 1);
    ...
    //double dist10 = top_node1->GetPos().z() - bot_node1->GetPos().z();
    double dist20 = top_node2->GetPos().z() - bot_node2->GetPos().z();
    //double dist30 = top_node3->GetPos().z() - bot_node3->GetPos().z(); 
    double dist40 = top_node4->GetPos().z() - bot_node4->GetPos().z();
	...
	while (sys.GetChTime() <= 0.2  ) {
		...
		if(stepnum%100==0) {
            ...
            //double dist11 = top_node1->GetPos().z() - bot_node1->GetPos().z();
            double dist22 = top_node2->GetPos().z() - bot_node2->GetPos().z();
            //double dist33 = top_node3->GetPos().z() - bot_node3->GetPos().z();
            double dist44 = top_node4->GetPos().z() - bot_node4->GetPos().z();

           // double strain1 = (dist11 - dist10) / dist10;
            double strain2 = (dist22 - dist20) / dist20;
            //double strain3 = (dist33 - dist30) / dist30;
            double strain4 = (dist44 - dist40) / dist40;

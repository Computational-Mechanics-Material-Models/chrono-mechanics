...
#include "FreshConcreteContact_V10.cpp"
...
/////////////////////////////////////////////////////////////////////////////////
struct ParticleHeightStats {
	double top_height;
	ChVector3d top_particle_location;
	double average_top_layer_height;
	int top_layer_particle_count;
};

ParticleHeightStats calculateParticleHeightStats(ChSystem& sys, double top_layer_depth) {
	ParticleHeightStats stats;
	stats.top_height = -std::numeric_limits<double>::max();
	stats.top_particle_location = ChVector3d(0, 0, 0);
	stats.average_top_layer_height = 0.0;
	stats.top_layer_particle_count = 0;
	...
	for (auto body : sys.GetBodies()) {
		...
		auto shape = body->GetCollisionModel()->GetShapeInstance(0).shape;
		auto shape_sphere = std::static_pointer_cast<ChCollisionShapeSphere>(shape);
		double particle_height = body->GetPos().y() + shape_sphere->GetRadius();
		ChVector3d particle_location = body->GetPos();

		particle_heights.push_back(particle_height);

		if (particle_height > stats.top_height) {
			stats.top_height = particle_height;
			stats.top_particle_location = particle_location;
		}
	}

	if (particle_heights.empty()) {
		stats.top_height = 0.0;
		return stats;
	}

	double top_layer_min_height = stats.top_height - top_layer_depth;
	double top_layer_height_sum = 0.0;
	for (double particle_height : particle_heights) {
		if (particle_height >= top_layer_min_height) {
			top_layer_height_sum += particle_height;
			stats.top_layer_particle_count++;
		}
	}

	if (stats.top_layer_particle_count > 0)
		stats.average_top_layer_height = top_layer_height_sum / stats.top_layer_particle_count;
...
std::shared_ptr<ChBody> AddTopWall(ChSystem& sys) {
    ...
    auto fixedBody=chrono_types::make_shared<ChBodyEasyBox>(70, 10, 70, 2.4e-9, true, true, fixed_mat);    
    fixedBody->SetFixed(true);
    fixedBody->SetPos(ChVector3d(0, 55, 0));
...
std::shared_ptr<ChBody> AddSlumpCone(ChSystem& sys, std::shared_ptr<ChContactMaterial> mat, double density, double radius, double height, std::string current_dir) {
	...
	auto SlumpCone = chrono_types::make_shared<ChBodyEasyMesh>(
        (current_dir+"/minislump-Cut4.obj").c_str(),
	...
	//ChQuaternion<> q=Q_from_AngAxis(CH_PI_2, -VECT_X); //mini slump
	ChQuaternion<> q;
	q.SetFromAngleAxis(CH_PI_2, -VECT_X); //mini slump
	SlumpCone->SetRot(q);
	std::cout << "COG : " << SlumpCone->GetFrameCOMToAbs().GetPos() << std::endl;
	SlumpCone->SetPos(ChVector3d(0,27,0));
	//SlumpCone->GetVisualShape(0)->GetMaterial(0)->SetOpacity(0.5);
	//SlumpCone->SetBodyFixed(true);
	//
...
//------------------------------------------------------------------
// Function to save object file mesh to Paraview VTK files
//------------------------------------------------------------------
void WriteObjectSolidVTK(const std::string& filename,
...
int main(int argc, char* argv[]) {
	...
	//
	//////////////////////////////////////////////////	
	const std::string out_dir = current_dir + "/OUT_MiniSlump/";
    ...
    vis->SetWindowTitle("MINI Slump");
    vis->Initialize();
    vis->AddLogo();
    vis->AddSkyBox("/chrono-concrete/data/skybox/");
    vis->AddLight(ChVector3d(200,200,-250), 20, ChColor(1.0,1.0,1.0));
	// ChColor(1.0, 1.0, 1.0));ChColor(255,255,255)
    //vis->AddTypicalLights();
    vis->AddCamera(ChVector3d(150, 200, -200));
	//
    ...
    auto floorBody = chrono_types::make_shared<ChBodyEasyBox>(200, 10, 200, 2.4e-9, true, true, floor_mat);
    ...
    mat->SetAdhesion(0.0);
    //mat->SetCohesion(0);	
	double cyl_radius=50;
	double cyl_height=50;
	double density=7.8E-9;
	//
	//
	//=== Slump Cone ==========
    auto SlumpConeBody =AddSlumpCone(sys, mat, density, cyl_radius, cyl_height, current_dir);
    SlumpConeBody->GetVisualShape(0)->SetOpacity(0.25f);
    SlumpConeBody->SetFixed(true);
    std::string SlumpCone_obj = (current_dir+"/minislump-Cut4.obj").c_str();	
    ChTriangleMeshConnected SlumpCone_mesh;
    SlumpCone_mesh.LoadWavefrontMesh(SlumpCone_obj, false, true);
    SlumpCone_mesh.RepairDuplicateVertexes(1e-9);
    SlumpConeBody->EnableCollision(true);	
    //
	//
	//volume of specimen
	double specimenVol=( CH_PI*pow(101.6/2,2)*50.8*2/3- CH_PI*pow(69.85/2,2)*50.8/3);
	...
	//
	std::ofstream height_file(out_dir + "MiniSlump_height_vs_time.csv");
	if (!height_file.is_open()) {
		std::cerr << "Error creating height output file " << out_dir + "MiniSlump_height_vs_time.csv" << std::endl;
		return 1;
	}
	height_file << "time,top_height,top_particle_x,top_particle_y,top_particle_z,average_top_layer_height,top_layer_particle_count\n";
	height_file << std::fixed << std::setprecision(10);
	height_file.flush();
	//
	std::ofstream stats_file(out_dir + "MiniSlump_stats_vs_time.csv");
	if (!stats_file.is_open()) {
		std::cerr << "Error creating stats output file " << out_dir + "MiniSlump_stats_vs_time.csv" << std::endl;
		return 1;
	}
	stats_file << "time,va0,aggVolFrac,KE\n";
	stats_file << std::fixed << std::setprecision(10);
	stats_file.flush();
	//
	//
	sprintf(filename, "%s/Slump_%06d.vtk", out_dir.c_str(), step_num);			
	WriteObjectSolidVTK(filename, SlumpCone_mesh, SlumpConeBody->GetFrameRefToAbs()); 
	//
    ...
    while (vis->Run() && sys.GetChTime()<1.0) {
		...
		//
		if (step_num%20==0){	
			timestep=std::min(timestep*=1.05, 5.0e-5);
		}
		//
		...
		if (step_num %800 ==0) {
		    //
		    sprintf(filename, "%s/MiniSlump_va_A%06d.vtk", out_dir.c_str(), step_num);
		    WriteParticlesVTK(sys, filename, mortar_layer, true);
		    sprintf(filename, "%s/MiniSlump_va_M%06d.vtk", out_dir.c_str(), step_num);
			...
			ParticleHeightStats height_stats = calculateParticleHeightStats(sys, mortar_layer);
			height_file << current_time << ","
			            << height_stats.top_height << ","
			            << height_stats.top_particle_location.x() << ","
			            << height_stats.top_particle_location.y() << ","
			            << height_stats.top_particle_location.z() << ","
		            << height_stats.average_top_layer_height << ","
		            << height_stats.top_layer_particle_count << "\n";
			height_file.flush();
			stats_file << current_time << ","
			           << va0 << ","
			           << aggVolFrac << ","
			           << KE << "\n";
			stats_file.flush();
            std::cout << " time: " << current_time << " va0: " << va0 << " aggVolFrac: " << aggVolFrac << " IE: " << IE << " KE: " << KE << " rho_new: " << rho_new << " top_height: " << height_stats.top_height << " avg_top_layer_height: " << height_stats.average_top_layer_height << "\n";
			//
	...
	while (  vis->Run() && sys.GetChTime()<t1+9.5) {
		...
		if (step_num %800 ==0) {
		    ...
		    stats_file << current_time << ","
		               ...
		               << KE << "\n";
		    stats_file.flush();
            std::cout << " time: " << current_time << " va0: " << va0 << " aggVolFrac: " << aggVolFrac << " IE: " << IE << " KE: " << KE << " top_height: " << height_stats.top_height << " avg_top_layer_height: " << height_stats.average_top_layer_height << "\n";
			//

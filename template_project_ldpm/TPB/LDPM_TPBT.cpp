...
bool TieNodeToTriFace(ChSystem& sys, std::shared_ptr<ChMeshSurface> m_surface, std::shared_ptr<ChNodeFEAxyzrot> m_node,  double max_dist=0.0001) {    
    ...
    for (const auto& face : m_surface->GetFaces()) {
        if (auto face_tetra = std::dynamic_pointer_cast<ChTetrahedronFace>(face)) {
            ...
            val = utils::PointTrianglePlaneDistance(
...
int main(int argc, char** argv) {
	...
	*/
    ...
    auto mystepper = chrono_types::make_shared<ChTimestepperHHT>(&sys);
        ...
        mystepper->SetAbsTolerances(1e-01, 1e-01);
        ...
        */
        ...
        for (int i = 0; i < my_mesh_Elas->GetNumElements(); ++i) {
            ...
            Wint_elas = Wint_elas + vol * (stressAvg.XX() * dstrain.XX() + stressAvg.YY() * dstrain.YY() +
                stressAvg.ZZ() * dstrain.ZZ() + stressAvg.GetXY() * dstrain.GetXY() + stressAvg.GetXZ() * dstrain.GetXZ() + stressAvg.GetYZ() * dstrain.GetYZ());
		...
		if(stepnum%100==0) {
            ...
            std::string mesh_filename_crack=out_dir+"crack"+std::to_string(stepnum)+".vtk";
	    	std::string vtk_filename_crack=out_dir+"Vtkcrack"+std::to_string(stepnum)+".vtk";
	    	WriteMesh1(my_mesh_C, mesh_filename_crack);
	    	WriteFrame1(my_mesh_C, mesh_filename_crack, vtk_filename_crack);
            ...
            */
            ...
            << "\texternal_work\t" << Wext<< "\tdissipated energy\t" << E_dis << "\tkinetic_energy\t" << Ek+Ek_elas << "\tkinetic_energy_LDPM\t" << Ek << "\t\n";

...
if (!std::filesystem::exists(out_dir)) {
    ...
    */
    ...
    // =====================================================================
    // LDPM material parameters for Cusatis et al. 2011 - Section 5.1
    // High-confinement triaxial compression, Fig. 5c
    // Units used here: MPa, mm, tonne/mm^3
    // =====================================================================
    my_mat->Set_density(2.290E-9);       // rho = 2290 kg/m^3 = 2.290e-9 tonne/mm^3

    my_mat->Set_E0(38636);               // E0 [MPa]
    my_mat->Set_alpha(0.25);             // alpha [-]

    my_mat->Set_sigmat(4.16);            // sigma_t [MPa]
    my_mat->Set_sigmas(11.232);          // sigma_s = 2.7 * sigma_t [MPa]
    my_mat->Set_nt(0.2);                 // n_t [-]
    my_mat->Set_lt(100);                 // l_t [mm]

    my_mat->Set_Ed(69931.2);             // Ed = 1.81 * E0 [MPa]
    my_mat->Set_sigmac0(120);            // sigma_c0 [MPa]
    my_mat->Set_Hc0(25886.1);            // Hc0 = 0.67 * E0 [MPa]
    my_mat->Set_Hc1(3863.6);             // Hc1 = 0.10 * E0 [MPa]

    my_mat->Set_beta(0.0);               // beta [-]
    my_mat->Set_kc0(3.8);                // kappa_c0 [-]
    my_mat->Set_kc1(1.2);                // kappa_c1 [-]
    my_mat->Set_kc2(5.0);                // kappa_c2 [-]
    my_mat->Set_kc3(0.1);                // kappa_c3 [-]

    my_mat->Set_mu0(0.4);                // mu_0 [-]
    my_mat->Set_muinf(0.0);              // mu_infinity [-]
    my_mat->Set_sigmaN0(600);            // sigma_N0 [MPa]
    //
...
for (unsigned int i = 0; i < my_mesh->GetNumNodes(); i++) {
   ...
   std::cout << "zmax = " << zmax << "\n";
    ...
    //RP1->SetX0(ChFrame<>(0, 0, 101.6));
    std::shared_ptr<ChNodeFEAxyzrot> RP2;
    //RP2->SetX0(ChFrame<>(0, 0, 0));
...
*/
	...
	*/
    ...
    // phase 1 - hydrostatic pressure loading 
    

   double P0 = 100.0;        // confining pressure
   double t0 = 0.2;          // time to reach P0


   double Tfinal = 1.6;


    double P_lateral = 0.0;
    double P_axial = 0.0;
    double F_top = 0.0;
    double F_lateral_total = 0.0;

    // Phase 2 displacement-control settings
    double v_axial_final = -50.0;         
    double t_vel_ramp = 0.2;             
    bool phase2_initialized = false;
    double z_phase2_start = 0.0;
    double vz_phase2_start = 0.0;        

    // Motor used only after Phase 2 starts. This replaces direct SetPos/SetPosDt calls.
    std::shared_ptr<ChLinkMotorLinearPosition> motor_top_plate;
...
for (int i = 0; i < my_mesh->GetNumElements(); ++i) {
 ...
 //while (vis->Run() & sys.GetChTime() <= 0.2) {
	while (sys.GetChTime() <= Tfinal  ) {
			
F_lateral_total = 0.0;

// applying lateral pressure and switching axial control
double t = sys.GetChTime();

if (t <= t0) {
    // Step 1: hydrostatic confinement - Force/pressure-controlled axial loading and lateral pressure increase together.

    P_lateral = P0 * t / t0;
    P_axial   = P_lateral;
    F_top = P_axial * A_specimen;
    top_plate_force->SetMforce(F_top);
}
else {
    // Step 2: triaxial loading - displacement-controlled
   
    P_lateral = P0;
    P_axial = 0.0;
    F_top = 0.0;
    top_plate_force->SetMforce(0.0);

    // Initialize the displacement-controlled phase only once.
    // add a Chrono motor at the current top-plate configuration.

    if (!phase2_initialized) {
        z_phase2_start = top_plate->GetPos().z();
        vz_phase2_start = top_plate->GetPosDt().z();

        motor_top_plate = chrono_types::make_shared<ChLinkMotorLinearPosition>();
        motor_top_plate->Initialize(
            top_plate,                 // moving body
            mtruss,                    // fixed ground
            ChFrame<>(top_plate->GetPos(), QUNIT)  
        );


        double v_phase2_initial = vz_phase2_start;  
        double a_vel = (v_axial_final - v_phase2_initial) / t_vel_ramp;
        double disp_ramp_end = v_phase2_initial * t_vel_ramp + 0.5 * a_vel * t_vel_ramp * t_vel_ramp;

        std::cout << "Phase 2 starts at t = " << t
                  << " s, z = " << z_phase2_start
                  << " mm, measured vz = " << v_phase2_initial
                  << " mm/s, ramping to " << v_axial_final
                  << " mm/s over " << t_vel_ramp << " s." << std::endl;

        auto zero_motion = chrono_types::make_shared<ChFunctionConst>(0.0);

        auto accel_motion = chrono_types::make_shared<ChFunctionPoly>();
        accel_motion->SetCoefficients(std::vector<double>{
            0.0,
            v_phase2_initial,
            0.5 * a_vel
        });

        auto const_velocity_motion = chrono_types::make_shared<ChFunctionRamp>(disp_ramp_end, v_axial_final);

        auto phase2_motion = chrono_types::make_shared<ChFunctionSequence>();
        phase2_motion->InsertFunct(zero_motion, t0, 1.0, true);
        phase2_motion->InsertFunct(accel_motion, t_vel_ramp, 1.0, true);
        phase2_motion->InsertFunct(const_velocity_motion, Tfinal + 10.0, 1.0, true);

        motor_top_plate->SetMotionFunction(phase2_motion);
        sys.Add(motor_top_plate);
...
for (auto bf : lateral_faces) {
    ...
    double F_face = P_lateral * A_face;
    F_lateral_total += F_face;
    ...
    for (auto node : face_nodes) {
        ...
        if (r > 1e-12) {
            ...
            lateral_force_map[node] += ChVector3d(-F_node * rx, -F_node * ry, 0.0);
        }
    }
}
...
for (int i = 0; i < my_mesh->GetNumElements(); ++i) {
    ...
    double avg_vol_strain = weighted_ev_sum / total_volume;    
        ...
        double sigma_z = support_Rz / A_specimen;
        double top_plate_vz = top_plate->GetPosDt().z();

std::cout << " t=\t" << sys.GetChTime()
          << "\ttop_disp_z=\t" << top_disp
         << "\tP_lateral=\t" << P_lateral
         << "\tP_axial_input=\t" << P_axial
         << "\tv_axial_final=\t" << v_axial_final
         << "\tv_phase2_initial_measured=\t" << vz_phase2_start
         << "\tF_lateral_total=\t" << F_lateral_total
         << "\tF_top=\t" << F_top
         << "\ttop_plate_vz=\t" << top_plate_vz
